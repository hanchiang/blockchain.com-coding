...
class Dependencies:
    order_book_service_by_exchange: dict[str, ThirdPartyOrderBookService] = {}
    order_book_transformer_by_exchange: dict[str, ThirdPartyOrderBookTransformer] = {}
    metadata_dao_by_storage: dict[str, MetadataDao] = {}
    metadata_parser_by_file_type: dict[str, MetadataParser] = {}
    ...
    def build_order_book_service():
        Dependencies.order_book_service_by_exchange = {
    ...
    def build_order_book_transformer():
        Dependencies.order_book_transformer_by_exchange = {
            "blockchain.com": BlockChainOrderBookTransformer()
        }

    @staticmethod
    def build_order_book_metadata_dao():
        Dependencies.metadata_dao_by_storage = {"memory": MetadataMemoryDao()}

    @staticmethod
    def build_order_book_metadata_parser():
        Dependencies.metadata_parser_by_file_type = {"csv": CsvMetadataParser()}

    @staticmethod
    def get_order_book_service(key: str) -> ThirdPartyOrderBookService:
        Dependencies.build()
        service = Dependencies.order_book_service_by_exchange.get(key, None)
        if service is None:
    ...
    def get_order_book_transformer(key: str) -> ThirdPartyOrderBookTransformer:
        Dependencies.build()
        transformer = Dependencies.order_book_transformer_by_exchange.get(key, None)
    ...
    def get_metadata_dao(key: str) -> MetadataDao:
        Dependencies.build()
        res = Dependencies.metadata_dao_by_storage.get(key, None)
        if res is None:
    ...
    def get_metadata_parser(key: str) -> MetadataParser:
        Dependencies.build()
        res = Dependencies.metadata_parser_by_file_type.get(key, None)
        if res is None:
            raise DependencyNotFoundException(
